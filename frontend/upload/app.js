...
window.onload = function () {
    ...
    document.getElementById("submitButton").onclick = async function (e) {
        ...
        let termRequired =
            document.getElementById("selectTerm").value != "Term";
        ...
        for (let i = 0; i < inputs.length; ++i) {
            if (
                ...
                !deadlines[i].children[0].value &&
                !weights[i].children[0].value
            )
                continue;

            if (!inputs[i].children[0].value)
                inputs[i].children[0].classList.add("required");
            if (!deadlines[i].children[0].value)
                deadlines[i].children[0].classList.add("required");
            if (
                !weights[i].children[0].value ||
                weights[i].children[0].value < 1 ||
                weights[i].children[0].value > 100
            )
                weights[i].children[0].classList.add("required");
            valid = false;
        }
    ...
    function addNewRow(name = "", deadline, weight = 1) {
        ...
        newEl.innerHTML = `
            ...
            <td class="tableCell deadline">
                <input class="tableInput" type="date" value="${
                    deadline || ""
                }" />
            </td>
